...
def summarize_label_mapping(
    ...
    metrics_rows = []
    for src_label, df_sub in assignments_df.groupby(label_col):
        n = len(df_sub)
    ...
    row_view = row_frac.loc[top_src, top_dst]
    count_view = counts.loc[top_src, top_dst]

    fig, axes = plt.subplots(1, 2, figsize=(16, max(6, 0.35 * len(top_src))))
    sns.heatmap(
        row_view,
        ax=axes[0],
        cmap="YlGnBu",
    ...
    axes[0].set_title(f"{prefix}: {label_col} -> assignment (row fraction)")
    ...
    axes[1].set_title(f"{prefix}: {label_col} -> assignment (counts)")
...
def run_stage5_10x_hmb(
    ...
    gating_df = gating_df.merge(
        tenx_aligned.obs[[label_col]].reset_index().rename(columns={"index": "cell_id"}),
        on="cell_id",
        how="left",
    )
